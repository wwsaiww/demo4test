...
public class ExcelUtil {
     ...
     *                       如：list中存放的都是student，student中又有college属性，而我们需要学院名称，则可以这样写
     *                       fieldMap.put("college.collegeName","学院名称")
     * @param collectionName 集合字段的中英文對應關係
     * @param title
     * @param content
     * @param sheetName      工作表的名称
     * @param sheetSize      每个工作表中记录的最大个数
     ...
     * @param sheet               工作表名称
     * @param list                数据源
     * @param normalFieldMap      普通中英文字段对应关系的Map
     * @param collectionFieldName 集合類中英文字段对应关系的Map
     * @param collectionFieldName 集合類中英文字段对应关系的Map
     * @param firstIndex          开始索引
     * @param lastIndex           结束索引
     * @MethodName    : fillSheet
    ...
    private static <T> void fillSheet(
        ...
        List childList = (List) getFieldValueByName(collectionFieldName, firstItem);
        ...
        for (int index = firstIndex; index <= lastIndex; index++) {
            ...
            if (collectionFieldName != null && !collectionFieldName.equals("")) {
                //拿到集合对象
                List currentList = (List) getFieldValueByName(collectionFieldName, item);
                //将集合对象行转列
                for (int i = 0; i < currentList.size(); i++) {
                    Object objValue = getFieldValueByNameSequence(content, currentList.get(i));
                    String fieldValue = objValue == null ? "" : objValue.toString();
                    Label label = new Label(i + cnFields.length, rowNo, fieldValue);
                    sheet.addCell(label);
                }
            }
